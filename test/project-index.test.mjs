...
test("project index never follows symlinks outside the canonical project root", async () => {
  const root = await mkdtemp(join(tmpdir(), "devmoter-index-symlink-"));
  const projectRoot = join(root, "project");
  const outside = join(root, "outside");
  try {
    await mkdir(projectRoot, { recursive: true });
    await mkdir(outside, { recursive: true });
    await writeFile(join(projectRoot, "safe.ts"), "export const safe = true;\n");
    await writeFile(join(outside, "secret.ts"), "export const stolenSecret = true;\n");
    await symlink(join(outside, "secret.ts"), join(projectRoot, "escape.ts"));

    const project = { id: "p3", name: "Symlink", path: projectRoot };
    const stateDir = join(root, "state");
    const index = createProjectIndex({ stateDir, resolveProject: async () => project });
    await index.rebuild(project.id);

    assert.equal((await index.search(project.id, "stolenSecret")).results.length, 0);
    assert.equal((await index.search(project.id, "safe")).results[0].path, "safe.ts");
