...
| MAR-13 API Vault | Partial | This branch adds a passphrase-unlocked AES-256-GCM host secret-store primitive with `secret://` references, project/provider binding, duplicate detection, metadata-only listing, and owner-only atomic file writes. Existing API Chat provider keys still use the legacy JSON store; HTTP/UI unlock, explicit reveal/re-auth, migration, provider injection, and end-to-end log/AI redaction remain. |
