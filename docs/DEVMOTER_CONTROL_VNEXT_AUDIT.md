...
| MAR-13 API Vault | Partial | The encrypted AES-256-GCM host secret store has a paired-device-gated HTTP management API and a mobile-first Settings page for initialization, unlock/lock, metadata listing, project-bound writes, confirmed replacement, and confirmed deletion. Existing API Chat provider keys still use the legacy JSON store; explicit reveal/re-auth, migration, provider injection, fresh reauthentication, and end-to-end log/AI redaction remain. |
| MAR-14 Security model | Partial | Auth, exact-origin mutation gate, optional passkeys, operation dedupe, agent/workspace policies, local-preview browser isolation, and active paired-device checks on Terminal and Secret Vault management APIs. A unified action permission model and broader device/resource ownership across Host/Browser remain. |
