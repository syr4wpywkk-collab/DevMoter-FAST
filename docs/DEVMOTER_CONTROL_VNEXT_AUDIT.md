...
- **Authentication and Origin:** single-user Basic authentication plus optional external owner sessions; mutations pass exact same-origin checks. Optional WebAuthn passkeys add a host/origin-scoped gate. The MAR-14 follow-up additionally requires an active paired-device token for terminal and workspace-control APIs; this is not multi-user authorization or device-bound identity for every resource.
- **Streaming:** On the audit baseline, OpenCode/Codex event streams and an HTTP/NDJSON terminal stream exist. The MAR-12 branch replaces `script` with `node-pty`, adds xterm.js and a same-origin WebSocket transport with bounded input/resize, and keeps the authenticated NDJSON routes for compatibility. WebSocket reconnection uses short-lived single-use tickets bound to session and Origin; named persistent sessions remain a separate MAR-15 requirement.
- **Host registry/protocol:** `server/control-plane.mjs` stores remote host labels, HTTP(S) addresses, trust state, schedules, and event triggers. This batch adds an authenticated v1 local Host discovery endpoint and Linux/macOS/Windows WSL adapter selection. Existing operations have not migrated behind capability adapters yet.
...
| MAR-21 Host abstraction | Partial | `GET /api/host` now provides a versioned OS-neutral capability snapshot, with Linux, Windows WSL, macOS, and disconnected Windows handling. Only capabilities already exposed by the Linux runtime are advertised; native macOS/Windows operations and capability API migration remain. |
