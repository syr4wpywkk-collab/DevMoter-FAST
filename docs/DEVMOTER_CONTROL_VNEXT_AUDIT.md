...
| MAR-13 API Vault | Partial | The encrypted AES-256-GCM host secret store has a paired-device-gated HTTP API, mobile Settings page, project-bound API Chat references, and explicit migration for editable file-backed providers on verified named presets. Environment/custom raw keys still need a migration path; Agent/Browser use, reveal/re-auth, comprehensive audit, and end-to-end log/AI redaction remain. |
