...
| MAR-13 API Vault | Partial | The encrypted AES-256-GCM host secret-store primitive now has a paired-device-gated HTTP management API for initialization, unlock/lock, metadata listing, project-bound writes, confirmed replacement, and confirmed deletion. Existing API Chat provider keys still use the legacy JSON store; UI, explicit reveal/re-auth, migration, provider injection, fresh reauthentication, and end-to-end log/AI redaction remain. |
