...
| MAR-13 API Vault | Partial | The encrypted AES-256-GCM host secret store has a paired-device-gated HTTP management API, mobile-first Settings page, and project-bound API Chat provider references resolved only on the Host. Existing raw API Chat provider keys still use the legacy JSON store; Agent/Browser use, explicit reveal/re-auth, migration, comprehensive audit, and end-to-end log/AI redaction remain. |
