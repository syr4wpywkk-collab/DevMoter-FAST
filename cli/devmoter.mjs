...
function usage() {
  return [
    ...
    "  --username <name>       Basic auth username (or DEVMOTER_AUTH_USERNAME)",
    "  --password-file <path>  Read Basic auth password from a private file",
    "",
    "Password lookup order:",
    "  --password-file, DEVMOTER_AUTH_PASSWORD, then the local DevMoter password file",
    "",
...
function isLocalUrl(value) {
  try {
    const host = new URL(String(value || "http://127.0.0.1:8787")).hostname.replace(/^\[|\]$/g, "").toLowerCase();
...
async function readPasswordFile(path) {
  ...
  if (!value) throw new Error("DevMoter password file is empty");
  return value;
}

export async function resolveCliAuth(options = {}, env = process.env) {
  const baseUrl = options.url || env.DEVMOTER_URL || "http://127.0.0.1:8787";
  const username = options.username || env.DEVMOTER_AUTH_USERNAME || "devmoter";

  if (options["password-file"]) {
    return {
      username,
      password: await readPasswordFile(String(options["password-file"]))
    };
  }
  if (env.DEVMOTER_AUTH_PASSWORD) {
    return { username, password: String(env.DEVMOTER_AUTH_PASSWORD) };
  }
  if (isLocalUrl(baseUrl)) {
    try {
      const path = join(
        ...
        "opencode-pocket",
        "devmoter-auth-password"
      );
      return { username, password: await readPasswordFile(path) };
    } catch {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  let auth;
  try {
    auth = await resolveCliAuth(options);
  } catch (error) {
    process.stderr.write((error instanceof Error ? error.message : String(error)) + "\n");
    return EXIT.USAGE;
  }
  ...
  try {
    ...
    if (command === "open") {
      ...
      if (options.json) writeJson(payload);
      else process.stdout.write(String(payload.url || "") + "\n");
      if (options.browser) {
    ...
    const authError = error instanceof DevMoterError && error.status === 401;
    process.stderr.write((error instanceof Error ? error.message : String(error)) + "\n");
    if (authError) {
      process.stderr.write("Provide DEVMOTER_AUTH_PASSWORD, --password-file, or use the local DevMoter password file.\n");
    }
