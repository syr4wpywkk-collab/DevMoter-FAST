import { appendFile, constants as FS, lstat, mkdir, open, readFile, readdir, realpath, rename, stat, writeFile } from "node:fs/promises";
...
async function readJsonFile(path, fallback) {
  try {
    ...
    throw new Error("Workspace control state is unreadable or malformed: " + path);
  }
}
...
function normalizeLockPath(value) {
  ...
  if (!raw || raw.includes("\0") || raw.startsWith("/") || /^[A-Za-z]:\//.test(raw)) {
    throw new Error("Lock path must be project-relative");
  }
  const normalized = posix.normalize(raw);
  if (normalized === "." || normalized === ".." || normalized.startsWith("../")) {
    throw new Error("Lock path escapes project");
  }
...
function assertInside(root, target) {
  ...
  if (rel === ".." || rel.startsWith(".." + sep) || isAbsolute(rel)) {
    throw new Error("Workspace path escapes registered project");
  }
}
...
export function createEventStore({ stateDir }) {
  ...
  function eventFile(sessionId) {
    return join(stateDir, "workspace-events", `${cleanId(sessionId)}.jsonl`);
  }

  async function hydrate(sessionId) {
    sessionId = cleanId(sessionId);
    if (memory.has(sessionId)) return;
    if (!hydration.has(sessionId)) {
      hydration.set(sessionId, (async () => {
        ...
        try {
          ...
          if (error?.code !== "ENOENT") {
            throw new Error("Workspace event history is unreadable or malformed");
          }
        }
        rows.sort((a, b) => Number(a.seq || 0) - Number(b.seq || 0));
        memory.set(sessionId, rows);
        counters.set(sessionId, rows.reduce((max, item) => Math.max(max, Number(item.seq || 0)), 0));
      })().finally(() => hydration.delete(sessionId)));
    }
    await hydration.get(sessionId);
  }
  ...
  async function waitQueue(sessionId) {
    await (queues.get(cleanId(sessionId)) || Promise.resolve()).catch(() => {});
  }
  ...
  async function appendUnlocked(sessionId, event = {}) {
    ...
    const dir = join(stateDir, "workspace-events");
    await mkdir(dir, { recursive: true, mode: 0o700 });
    await appendFile(eventFile(sessionId), `${JSON.stringify(record)}\n`, { mode: 0o600 });
    return record;
  }
  ...
  return {
    async append(sessionId, event = {}) {
      sessionId = cleanId(sessionId);
      return enqueue(sessionId, () => appendUnlocked(sessionId, event));
    },
    ...
    async compact(sessionId, { keepRecent = 80 } = {}) {
      sessionId = cleanId(sessionId);
...
async function secureDirectory(root, candidate) {
  const lexical = resolve(candidate);
  assertInside(root, lexical);
  const info = await lstat(lexical);
  if (info.isSymbolicLink() || !info.isDirectory()) throw new Error("Unsafe structural-search directory");
  const actual = await realpath(lexical);
  assertInside(root, actual);
  return actual;
}

async function secureReadSource(root, candidate) {
  const lexical = resolve(candidate);
  assertInside(root, lexical);
  const before = await lstat(lexical);
  if (before.isSymbolicLink() || !before.isFile()) throw new Error("Unsafe structural-search file");
  const actual = await realpath(lexical);
  assertInside(root, actual);

  const handle = await open(actual, FS.O_RDONLY | (FS.O_NOFOLLOW || 0));
  try {
    const opened = await handle.stat();
    const after = await realpath(lexical);
    assertInside(root, after);
    if (after !== actual) throw new Error("Structural-search path changed during read");
    const current = await stat(after);
    if (Number(current.dev) !== Number(opened.dev) || Number(current.ino) !== Number(opened.ino)) {
      throw new Error("Structural-search file changed during read");
    }
    if (opened.size > MAX_INDEX_FILE_BYTES) return null;
    return {
      path: actual,
...
async function walkProjectFiles(projectRoot) {
  ...
  async function walk(candidateDir) {
    if (files.length >= MAX_INDEX_FILES) return;
    let dir;
    try {
      dir = await secureDirectory(root, candidateDir);
    } catch {
      return;
    }
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          const checked = await secureReadSource(root, full);
          if (checked) files.push(checked);
        } catch {
          // Ignore files that fail canonical containment or race revalidation.
        }
      }
    }
  }
...
export async function structuralSearch(projectRoot, query, { limit = 50 } = {}) {
  ...
  for (const file of files) {
    ...
    for (const symbol of parseSymbols(file.source, extname(file.path).toLowerCase(), rel)) {
      if (
...
export function createWorkspaceControl({ stateDir, resolveProject, policyRules = [] }) {
  const policy = createPolicyEngine(policyRules);
  const locks = createLockManager();
  const events = createEventStore({ stateDir });
  const settingsFile = join(stateDir, "workspace-control.json");
  const microtasksFile = join(stateDir, "workspace-microtasks.json");
  const auditFile = join(stateDir, "workspace-policy-audit.jsonl");
...
export async function handleWorkspaceControlRequest(req, res, url, control, { authenticateDevice } = {}) {
  ...
  try {
    ...
    if (req.method === "POST" && path === "/api/workspace-control/policy/decide") {
      sendJson(res, 200, await control.decide(await readRequestJson(req)));
      return true;
    }

    if (req.method === "PUT" && path === "/api/workspace-control/policy") {
    ...
    if (req.method === "GET" && path === "/api/workspace-control/locks") {
      sendJson(res, 200, { locks: control.locks.list() });
      return true;
    }

    if (req.method === "POST" && path === "/api/workspace-control/locks/acquire") {
      const result = control.locks.acquire(await readRequestJson(req));
      sendJson(res, result.ok ? 200 : 409, result);
      return true;
    }

    if (req.method === "POST" && path === "/api/workspace-control/locks/release") {
      const result = control.locks.release(await readRequestJson(req));
      sendJson(res, result.ok ? 200 : 409, result);
      return true;
    }

    if (req.method === "GET" && path === "/api/workspace-control/events") {
    ...
    if (req.method === "POST" && path === "/api/workspace-control/events") {
    ...
    if (req.method === "POST" && path === "/api/workspace-control/events/compact") {
      const payload = await readRequestJson(req);
      sendJson(
        res,
        200,
    ...
    if (req.method === "GET" && path === "/api/workspace-control/search/symbols") {
    ...
    if (req.method === "POST" && path === "/api/workspace-control/deploy/preview") {
      sendJson(res, 200, await control.previewDeployment(await readRequestJson(req)));
      return true;
    }

    if (req.method === "POST" && path === "/api/workspace-control/deploy") {
...
export const workspaceControlInternals = { normalizeLockPath, secureReadSource };
