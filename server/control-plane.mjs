...
export function createLockManager({ now = () => Date.now(), defaultTtlMs = DEFAULT_LOCK_TTL_MS } = {}) {
  ...
  return {
    ...
    acquire({ projectId, path, owner, reason = "edit", ttlMs = defaultTtlMs }) {
      prune();
      const key = `${cleanId(projectId)}:${String(path)}`;
...
export function createEventStore({ stateDir }) {
  ...
  return {
    ...
    async compact(sessionId, { keepRecent = 80 } = {}) {
      ...
      const event = await this.append(sessionId, {
        type: "compaction",
        payload: {
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          if ((await stat(full)).size <= MAX_INDEX_FILE_BYTES) files.push(full);
        } catch {}
      }
    }
  }
...
export async function structuralSearch(projectRoot, query, { limit = 50 } = {}) {
  ...
  for (const full of files) {
    ...
    for (const symbol of parseSymbols(source, extname(full).toLowerCase(), rel)) {
      if (
