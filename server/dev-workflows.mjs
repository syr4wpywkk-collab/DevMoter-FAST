...
async function safeProjectEntry(projectPath, relativePath, { directory = false } = {}) {
  const root = await realpath(projectPath);
  const target = resolve(root, relativePath);
  if (!inside(root, target)) throw new Error("Project content path escapes registered project");
  const info = await lstat(target);
  if (info.isSymbolicLink()) throw new Error("Symlinked project workflow content is not allowed");
  const actual = await realpath(target);
  if (!inside(root, actual)) throw new Error("Project content escapes registered project through symlink");
  if (directory ? !info.isDirectory() : !info.isFile()) {
    throw new Error(directory ? "Project workflow path is not a directory" : "Project workflow path is not a regular file");
  }
  return { root, target: actual, info };
}

async function readProjectSmall(projectPath, relativePath, limit) {
  const entry = await safeProjectEntry(projectPath, relativePath);
  if (entry.info.size > limit) throw new Error("File is unavailable or too large");
  return readFile(entry.target, "utf8");
}

async function projectSkills(projectPath) {
  let dir;
  try {
    dir = await safeProjectEntry(projectPath, join(".devmoter", "skills"), { directory: true });
  } catch (error) {
    if (error?.code === "ENOENT") return [];
    return [];
  }
  const entries = await readdir(dir.target, { withFileTypes: true });
  const out = [];
  for (const entry of entries.slice(0, 50)) {
    if (!entry.isFile() || !/\.md$/i.test(entry.name)) continue;
    try {
      const relativePath = join(".devmoter", "skills", entry.name);
      const body = await readProjectSmall(projectPath, relativePath, MAX_SKILL);
      out.push({
        ...
        path: join(dir.target, entry.name),
        trusted: false,
        precedence: 10
      });
    } catch {}
  ...
  for (const name of names) {
    try {
      out.push({
        scope: "project",
        source: name,
        content: await readProjectSmall(projectPath, name, MAX_RULE),
        trusted: false,
        precedence: 10
      });
    } catch {}
...
async function projectExtension(projectPath) {
  try {
    const body = await readProjectSmall(projectPath, "devmoter.extension.json", 131072);
    ...
    return {
      present: true,
      valid: false,
      errors: [String(error?.message || "Project extension manifest is unavailable")],
      source: "devmoter.extension.json"
    };
  }
}
...
export const devWorkflowInternals = { readProjectSmall, safeProjectEntry };
