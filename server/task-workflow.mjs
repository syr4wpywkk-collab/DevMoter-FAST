...
async function projectPath(project, rel, allowMissing = false) {
  ...
  while (true) {
    try {
      ...
      if (info.isSymbolicLink()) throw new Error("Symlink paths are not allowed in reviewed changes");
      const actual = await realpath(cursor);
      if (!inside(root, actual)) throw new Error("Path escapes project through symlink");
      if (cursor === target) return { path, target: actual, root, exists: true, info };
      return { path, target, root, exists: false, parent: actual };
    } catch (error) {
      if (error?.code !== "ENOENT") throw error;
      if (!allowMissing && cursor === target) throw error;
      const parent = dirname(cursor);
      if (parent === cursor) throw new Error("Could not resolve project path");
      cursor = parent;
    }
  }
}
...
async function remoteBranchExists(cwd, branch) {
  ...
  try {
    ...
    if (Number(error?.code) === 2) return false;
    const stderr = String(error?.stderr || "").trim();
    throw new Error(stderr.slice(0, 600) || "Could not verify remote branch availability");
  }
}
...
export function createTaskWorkflow({ homeDir, getProjectById }) {
  ...
  async function createChange(projectId, input) {
    return locked(async () => {
      ...
      for (const item of files) {
        ...
        let original = "", originalExists = true, originalMode = null;
        try {
          const info = await lstat(p.target);
          if (info.isSymbolicLink()) throw new Error("Symlink paths are not allowed in reviewed changes");
        ...
        let hunks = lcsHunks(original, proposed ?? "").map(h => ({ ...h, decision: "pending" }));
        if (!hunks.length && ((!originalExists && proposed !== null) || (originalExists && proposed === null))) {
          hunks = [{ id: "h1", oldStart: 1, oldLines: 0, newStart: 1, newLines: 0, oldText: "", newText: "", decision: "pending" }];
        }
        normalized.push({ path: p.path, originalExists, originalMode, originalHash: hash(original), original, proposed, decision: "pending", hunks });
      }
  ...
  async function commitChange(projectId, changeId, input = {}) {
    ...
    for (const path of files) {
      ...
      if (!file) throw new Error("Applied file metadata is missing: " + path);
      const resolved = await projectPath(project, path, true);
      let exists = true;
      let current = "";
      try {
        const info = await lstat(resolved.target);
        if (info.isSymbolicLink() || !info.isFile()) throw new Error("Applied path is no longer a regular file: " + path);
        current = await readFile(resolved.target, "utf8");
      } catch (error) {
        if (error?.code === "ENOENT") exists = false;
        else throw error;
      }
      if (exists !== file.appliedExists || (exists && hash(current) !== file.appliedHash)) {
        throw new Error("Reviewed content changed after apply; review again before commit: " + path);
      }
    }
    const dirty = statusPaths((await run("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], project.path)).stdout);
  ...
  async function applyChange(projectId, changeId, input = {}) {
    return locked(async () => {
      ...
      for (const plan of plans) {
        const currentPath = await projectPath(project, plan.file.path, true);
        if (plan.next === null) {
          await rm(currentPath.target);
          plan.file.appliedExists = false;
          plan.file.appliedHash = null;
        } else {
          await mkdir(dirname(currentPath.target), { recursive: true, mode: 0o700 });
          const temp = currentPath.target + "." + process.pid + "." + randomUUID() + ".tmp";
          const mode = plan.file.originalExists && Number.isInteger(plan.file.originalMode) ? plan.file.originalMode : 0o600;
          await writeFile(temp, plan.next, { mode });
          await chmod(temp, mode);
          await rename(temp, currentPath.target);
          plan.file.appliedExists = true;
          plan.file.appliedHash = hash(plan.next);
        }
      }
  ...
  async function listWorktrees(projectId) {
    ...
    return await Promise.all(state.worktrees.filter(w => w.projectId === projectId && w.status === "active").map(async w => {
      try { return { ...w, dirty: Boolean((await run("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], w.path)).stdout.trim()) }; }
  ...
  async function cleanupWorktree(projectId, id, input = {}) {
    return locked(async () => {
      ...
      const dirty = statusPaths((await run("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], worktree.path)).stdout);
  ...
  async function pullRequest(projectId, input) {
    return locked(async () => {
      ...
      if ((await run("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], worktree.path)).stdout.trim()) throw new Error("Task worktree must be clean before creating a pull request");
      ...
      if (await remoteBranchExists(worktree.path, head)) {
        throw new Error("Remote branch already exists; refusing to overwrite or reuse it: " + head);
      }
...
export const workflowInternals = { lcsHunks, applyHunks, trimPreview, validBranch, statusPaths };
