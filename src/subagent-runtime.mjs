...
export class SubagentRuntime {
  constructor({ adapters = {}, idFactory = defaultId, policy = {} } = {}) {
    ...
    this.policy = {
      maxDepth: Math.min(8, nonNegativeInt(policy.maxDepth, 3)),
  ...
  async cancelFleet(id) {
    ...
    await Promise.all(active.map(run => this.cancel(run.id).catch(error => {
      if (!fleet.errors.some(item => item.runId === run.id)) {
  ...
  async spawn(spec = {}) {
    ...
    if (this.runs.has(runId)) throw new Error(`Duplicate subagent run id: ${runId}`);
    ...
    try {
      ...
      if (started && typeof started === "object") {
        const safePatch = { ...started };
        if (current && TERMINAL_RUN_STATES.has(current.state)) delete safePatch.state;
        this.update(run.id, safePatch);
      }

      const latest = this.runs.get(run.id);
      const fleetCancelled = latest?.fleetId
        ? this.fleets.get(latest.fleetId)?.state === "cancelled"
        : false;
      if (latest?.cancelRequested || latest?.state === "cancelled" || fleetCancelled) {
        if (latest && latest.state !== "cancelled") {
          this.update(run.id, { state: "cancelled", cancelRequested: true, pendingApproval: null });
        }
        await adapter.cancel(this.getRun(run.id)).catch(() => {});
        return this.getRun(run.id);
      }

      if (latest?.state === "starting") this.update(run.id, { state: "running" });
      return this.getRun(run.id);
    } catch (error) {
      if (parentRun) {
        parentRun.budget.tokensRemaining += tokenLimit;
  ...
  update(id, patch = {}) {
    ...
    if (
      ...
      next.state &&
      !TERMINAL_RUN_STATES.has(next.state)
    ) {
      delete next.state;
    }
    if (run.state === "cancelled" && next.state && next.state !== "cancelled") {
      delete next.state;
    }

    Object.assign(run, next, { updatedAt: Date.now() });

    if (run.fleetId && run.state === "failed") {
      const fleet = this.fleets.get(run.fleetId);
      if (fleet && !fleet.errors.some(item => item.runId === run.id)) {
        fleet.errors.push({
          runId: run.id,
  ...
  async cancel(id) {
    ...
    await adapter.cancel(this.getRun(run.id)).catch(error => {
      const latest = this.runs.get(run.id);
      if (latest?.sessionId && latest?.turnId) throw error;
    });
    return this.getRun(run.id);
  }
  ...
  async #pumpFleet(id) {
    ...
    fleet.pumping = true;
    try {
      while (fleet.queue.length && fleet.state === "running") {
      ...
      const activeCount = fleet.runIds
        .map(runId => this.runs.get(runId))
        .filter(run => run && !TERMINAL_RUN_STATES.has(run.state)).length;
      if (fleet.state === "running" && fleet.queue.length === 0 && activeCount === 0) {
...
export function createCodexSubagentAdapter({
  ...
  return {
    async start(run, emit) {
      ...
      if (run.context?.projectId) threadParams.projectId = String(run.context.projectId);
