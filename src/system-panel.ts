...
const interestingStates = new Set(["completed", "waiting_for_approval", "waiting_for_input"]);
...
export function mountSystemPanel() {
  ...
  async function notify(detail: AgentStateDetail) {
    if (!token() || !interestingStates.has(String(detail.state || ""))) return;
    await request("/api/push/notify", {
      method: "POST",
      body: JSON.stringify({
  ...
  window.addEventListener("devmoter-agent-state", ((event: CustomEvent<AgentStateDetail>) => {
    const detail = event.detail || {};
    activeSessionId = String(detail.sessionId || "");
    activeBackend = detail.backend === "codex" ? "codex" : "opencode";
    void syncVisibility();
    void notify({ ...detail, backend: activeBackend });
  }) as EventListener);
