...
const RICH_SELECTOR =
  ".cx-message-row.assistant .cx-message-text, .ocx-message-row.assistant:not(.live) .ocx-assistant-text";
...
function createToolbar() {
  ...
  const activeSessionId = () =>
    document.querySelector<HTMLElement>("[data-session-id]")?.dataset.sessionId ||
    localStorage.getItem("opencode-pocket-session") ||
  ...
  async function requestCompaction(mode: "manual" | "auto", status?: HTMLElement) {
    ...
    try {
      const res = await fetch("/api/control/events/compact", {
        method: "POST",
  ...
  tasksButton.addEventListener("click", async () => {
    ...
    try {
      const res = await fetch(
        `/api/control/microtasks?parentId=${encodeURIComponent(parentId)}`
      );
      const payload = await res.json();
      ...
      for (const [label, statusValue] of choices) {
        ...
        button.addEventListener("click", async () => {
          ...
          const save = await fetch("/api/control/microtasks", {
            method: "PUT",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
              parentId,
      ...
      for (const item of items) {
        ...
        button.addEventListener("click", async () => {
          const change = await fetch(
            `/api/control/extensions/${encodeURIComponent(String(item.id))}`,
