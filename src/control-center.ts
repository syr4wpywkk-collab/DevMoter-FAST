...
const XTERM_FIT_URL = "https://cdn.jsdelivr.net/npm/@xterm/addon-fit@0.10.0/+esm";
...
export function mountControlCenter() {
  ...
  async function ensureTerminalRenderer() {
    if (terminalInstance || terminalFallback) return;
    ...
    try {
      ...
      const xtermModule: { Terminal?: new (options?: Record<string, unknown>) => XtermInstance } =
        await import(/* @vite-ignore */ XTERM_URL);
      if (!xtermModule.Terminal) throw new Error("xterm Terminal export missing");

      terminalInstance = new xtermModule.Terminal({
        cursorBlink: true,
        convertEol: false,
        fontSize: 13,
        scrollback: 5000,
      ...
      terminalInstance.open(terminalShell);

      try {
        const fitModule: { FitAddon?: new () => { fit?: () => void } } =
          await import(/* @vite-ignore */ XTERM_FIT_URL);
        if (fitModule.FitAddon && terminalInstance.loadAddon) {
  ...
  async function reattachTerminal() {
    ...
    await ensureTerminalRenderer();
    if (!payload.session.closed) void attachTerminalStream();
  }
