...
type ComposerDispatchOptions = {
  mode?: "build" | "plan" | "ask" | "review";
  model?: string | null;
  provider?: string | null;
};

function dispatchToActiveComposer(prompt: string, options: ComposerDispatchOptions = {}) {
  const candidates = [
    {
  ...
  if (!target?.input || !target.form || !target.send) {
    throw new Error("Active DevMoter composer was not found");
  }

  const busy =
    target.send.classList.contains("stop") ||
    target.send.getAttribute("aria-label") === "Stop" ||
    target.send.getAttribute("aria-label") === "停止";

  if (busy) throw new Error(`${target.backend} is busy; wait for the current run to finish`);
  if (target.input.disabled || target.send.disabled) {
    throw new Error(`${target.backend} is offline or reconnecting`);
  }

  let provider = String(options.provider || "").trim();
  let model = String(options.model || "").trim();
  if (target.key === "opencode" && model && !provider && model.includes("/")) {
    const slash = model.indexOf("/");
    provider = model.slice(0, slash);
    model = model.slice(slash + 1);
  }
  if (target.key === "opencode" && model && !provider) {
    throw new Error("OpenCode model override requires a provider or provider/model value");
  }
  if (
    target.key === "codex" &&
    provider &&
    !["openai", "codex"].includes(provider.toLowerCase())
  ) {
    throw new Error(`Provider ${provider} is not compatible with the active Codex composer`);
  }

  if (options.mode) target.input.dataset.devmoterModeOverride = options.mode;
  else delete target.input.dataset.devmoterModeOverride;
  if (model) target.input.dataset.devmoterModelOverride = model;
  else delete target.input.dataset.devmoterModelOverride;
  if (provider) target.input.dataset.devmoterProviderOverride = provider;
  else delete target.input.dataset.devmoterProviderOverride;
...
function dispatchMode(mode: ModeConfig): "build" | "review" {
  return mode.mutationPolicy === "read-only-until-explicit-transition" || mode.id === "review"
    ? "review"
    : "build";
}
...
export function mountAgentConsole() {
  ...
  function renderModelRouting() {
    ...
    for (const role of AGENT_ROLES) {
      ...
      `;
      row.querySelector<HTMLSelectElement>("[data-route-backend]")!.value = route?.backend || "codex";
  ...
  function routedExecution(role: AgentRole, fallback?: string | null) {
    const route = resolveRoleModel(role, modelRoutes);
    const model = route.model || fallback || undefined;
    const backend = route.backend || "codex";
    const provider = route.provider || null;

    if (backend === "codex" && provider && !["openai", "codex"].includes(provider.toLowerCase())) {
      throw new Error(`Role ${role} routes provider ${provider} to Codex, which is unsupported`);
    }
  ...
  function updateDashboardSummary() {
    const runs = subagents.listRuns().filter(run => run.kind !== "second-opinion");
  ...
  function renderSubagentRun(run: SubagentRun) {
    ...
    if (run.sessionId) {
      ...
      open.addEventListener("click", () => {
        ...
        url.searchParams.set("backend", "codex");
        url.searchParams.set("session", run.sessionId!);
  ...
  opinionLaunch.addEventListener("click", () => {
    void (async () => {
      try {
        ...
        const reviewer = routedExecution("reviewer");
      ...
      const next = [...modelRouteRows.querySelectorAll<HTMLElement>(".devmoter-model-route-row")]
        .map(row => {
          ...
          return {
            role,
            model,
            backend: row.querySelector<HTMLSelectElement>("[data-route-backend]")!.value,
  ...
  subagentRun.addEventListener("click", () => {
    void (async () => {
      try {
        ...
        const mode = resolveMode(activeModeId, customModes);
        const role = selectedRole(activeModeId);
        const route = routedExecution(role, mode.model);
      ...
      try {
        ...
        const fleet = await subagents.runFleet(
          orchestrationPlan.children.map(child => {
            const route = routedExecution(child.owner as AgentRole, mode.model);
            return {
              backend: route.backend,
              parentSessionId: parent,
              role: child.owner,
  ...
  orchestrationApprove.addEventListener("click", () => {
    try {
      if (!orchestrationPlan) throw new Error("Preview a decomposition first");
      const approvedPreview = structuredClone(orchestrationPlan);
      approveOrchestrationPlan(approvedPreview);
      const backend = dispatchToActiveComposer(
        renderOrchestrationPrompt(approvedPreview),
        { mode: "build" }
      );
