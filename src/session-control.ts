...
export function mountSessionControl(options: MountOptions = {}) {
  ...
  async function stop(s:Session){try{if(s.backend==="codex"){let turn=s.activeTurnId;if(!turn){const p=await codex<Json>("thread/read",{threadId:s.id,includeTurns:true});turn=[...(p?.thread?.turns??[])].reverse().find((x:Json)=>["running","