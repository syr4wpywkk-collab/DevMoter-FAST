...
- integration action/status routes use authenticated same-origin POST because those routes launch integrations or perform stateful discovery. Installer v2 `GET /api/setup/status` is a read-only localhost-only exception for fixed version/auth-status probes. `POST /api/setup/plan` is also localhost-only, authenticated, and same-origin; it validates only fixed tool/action enums, rescans state, returns `executable: false` plan data, and has no installer/execution capability;
...
Phase 2 adds `POST /api/setup/plan` behind the same owner-auth, exact-Origin, optional-passkey, and loopback peer/Host checks. Its bounded request schema contains only known tool IDs and fixed desired actions. The planner rejects extra fields, duplicate IDs, unknown actions, and unsupported installs; it does not merge browser objects into adapter metadata. Source, privilege, notes, expected changes, and verification are projected from server-owned adapter definitions. The deterministic response contains no command/argv, package name, URL, executable path, cwd, or environment and is explicitly non-executable. No child process is started for planning beyond the existing read-only Phase 1 detector probes. Installation, sudo, login initiation, downloads, and Tailscale changes remain out of scope.
