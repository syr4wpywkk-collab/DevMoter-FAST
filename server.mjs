...
void multiApiAttachments.cleanup().catch(error => console.warn("multi-api attachment cleanup failed", error));
...
async function readBodyBuffer(req, limit = 15 * 1024 * 1024) {
  const chunks = [];
  let size = 0;
...
async function multiApiChat(req, res) {
  try {
    ...
    const providers = await multiApiStore.listResolved();
    const result = await runMultiApiChat(providers, payload, fetch, {
      attachmentStore: multiApiAttachments
    });
    json(res, 200, result);
  } catch (error) {
    json(res, multiApiErrorStatus(error), {
