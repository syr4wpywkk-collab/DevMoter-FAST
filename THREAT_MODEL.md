...
| DEVICE/TRUST | Optional host/origin-scoped passkeys; Terminal and Secret Vault management APIs require an active paired-device credential; remote host registry has a separate trust label | Extend device/resource ownership and revocation consistently across Host and Browser APIs. Host “trusted” label alone is not authority. |
...
| Secret exfiltration / confused deputy | Encrypted Host vault, opaque references, paired-device-gated metadata/write API, mobile UI with no Reveal/Copy path, and no public HTTP resolve route; provider key omitted from public provider record | Migrate provider credentials, add policy-gated injection, fresh reauthentication for sensitive disclosure, audit coverage, and complete log/context redaction. A same-origin script compromise can still act with the paired-device token and read in-progress form inputs. |
