...
| DEVICE/TRUST | Optional host/origin-scoped passkeys; Terminal, Secret Vault management, and Vault-backed API Chat provider operations require an active paired-device credential; remote host registry has a separate trust label | Extend device/resource ownership and revocation consistently across Agent, Host, and Browser APIs. Host “trusted” label alone is not authority. |
...
| Secret exfiltration / confused deputy | Encrypted Host vault, opaque references, paired-device-gated metadata/write API, project/preset checks before Host-side API Chat resolution, key sent only in upstream auth header, response redaction, mobile UI with no Reveal/Copy path, and no public HTTP resolve route | Migrate legacy raw provider credentials; extend the same server policy to Agent/Browser use; add fresh reauthentication for sensitive disclosure, audit coverage, and complete log/context redaction. A same-origin script compromise can still act with the paired-device token and read in-progress form inputs. |
