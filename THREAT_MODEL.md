...
| Secret exfiltration / confused deputy | Encrypted Host vault, opaque references, paired-device-gated metadata/write API, and no public HTTP resolve route; provider key omitted from public provider record | Migrate provider credentials, add policy-gated injection, fresh reauthentication for sensitive disclosure, audit coverage, and complete log/context redaction. |
